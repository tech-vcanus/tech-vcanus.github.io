...
```
vcpkg install openssl zlib curl
```
- 시스템 속성->환경 변수->시스템 변수 추가 후 재부팅
- 변수명: OPEN_SSL_ROOT, 경로명: C:\vcpkg-master\installed\x86-windows (depends on your system)
...
- example 디렉토리에 lib , dll 파일 생성되고, 이 파일들을 
...
C:\vcpkg-master\installed\x86-windows\include;
