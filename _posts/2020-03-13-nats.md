...
```
* 설치 : openssl , cmake , git clone https://github.com/nats-io/nats.c.git
...
cmake .. -DNATS_BUILD_STREAMING=OFF (nats Streaming API 빌드 하지 않음)
make
결과 : /usr/local/lib -> nats 관련 라이브러리 생성 됨
      /usr/local/include -> nats 관련 include 생성 됨
...
Undefined symbols for architecture x86_64:
  ...
  "_X509_VERIFY_PARAM_set_hostflags", referenced from:
      __processConnInit in conn.c.o
* 원인 : openssl 라이브러리 찾지 못함
* 해결 :
openssl 라이브러리 절대경로를 cmake 할떄 명령어로 지정해 준다.
ex) cmake .. -DNATS_BUILD_STREAMING=OFF -DOPENSSL_SSL_LIBRARY=/usr/local/opt/openssl@1.1/lib/libssl.dylib -DOPENSSL_CRYPTO_LIBRARY=/usr/local/opt/openssl@1.1/lib/libcrypto.dylib
```