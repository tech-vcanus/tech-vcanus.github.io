...
참고자료: [AI-Thinker Nodemcu-32s datasheet](http://wiki.ai-thinker.com/_media/esp32/docs/nodemcu-32s_product_specification.pdf)<br>
...
cf. MCP4725 DAC 모듈의 경우, 기본 주소값이 0x62이다. 마찬가지로 위의 코드를 실행했을 때 `I2C device found at address 0x62`라는 메시지가 출력되어야 한다.<br>

※주의: 아두이노 코드의 setup()에서 Serial.begin() 함수 괄호 안의 값은 baud rate이다. 시리얼 모니터 또는 플로터 실행 시 하단의 baud rate을 Serial.begin()에 쓴 값으로 맞춰야 확인하고자 하는 값이 제대로 출력된다.
